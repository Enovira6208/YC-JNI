 ...
 * @param size
 */
char *JHLK_100_AnalyDataCnt(uint8_t *buff, uint16_t size)
{
    JHLK_100_DataCnt = (buff[0] << 8) | (buff[1]);
...
char *JHLK_100_DataAnalysis(uint8_t *buffer, uint16_t size)
{
    ...
    for (int j = 0; j < 48; j++) {
        SerialNumber_int = SerialNumber_int + JHLK_100_Value.SerialNumber[j] * pow(10, 47 - j);
    }

    JHLK_100_Value.TestEncoding[0] = recv->TestEncoding[0];
    JHLK_100_Value.TestEncoding[1] = recv->TestEncoding[1];
    JHLK_100_Value.TestEncoding[2] = recv->TestEncoding[2];
    JHLK_100_Value.TestEncoding[3] = recv->TestEncoding[3];
    JHLK_100_Value.TestEncoding[4] = recv->TestEncoding[4];
    JHLK_100_Value.TestEncoding[5] = recv->TestEncoding[5];
    JHLK_100_Value.TestEncoding[6] = recv->TestEncoding[6];
    JHLK_100_Value.TestEncoding[7] = recv->TestEncoding[7];
    for (int j = 0; j < 8; j++) {
        TestEncoding_int = TestEncoding_int + JHLK_100_Value.TestEncoding[j] * pow(10, 7 - j);
    }

    JHLK_100_Value.JobNumber[0] = recv->JobNumber[0];
    JHLK_100_Value.JobNumber[1] = recv->JobNumber[1];
    JHLK_100_Value.JobNumber[2] = recv->JobNumber[2];
    JHLK_100_Value.JobNumber[3] = recv->JobNumber[3];
    JHLK_100_Value.JobNumber[4] = recv->JobNumber[4];
    JHLK_100_Value.JobNumber[5] = recv->JobNumber[5];
    JHLK_100_Value.JobNumber[6] = recv->JobNumber[6];
    JHLK_100_Value.JobNumber[7] = recv->JobNumber[7];
    for (int j = 0; j < 8; j++) {
        JobNumber_int = JobNumber_int + JHLK_100_Value.JobNumber[j] * pow(10, 7 - j);
    }

    JHLK_100_Value.SF6 = PUBLIC_IEEE754_32(recv->SF6[3], recv->SF6[2], recv->SF6[1], recv->SF6[0]);

    JHLK_100_Value.temperature = PUBLIC_IEEE754_32(recv->temperature[3], recv->temperature[2], recv->temperature[1], recv->temperature[0]);
    JHLK_100_Value.humidity = recv->humidity;
    JHLK_100_Value.max = PUBLIC_IEEE754_32(recv->max[3], recv->max[2], recv->max[1], recv->max[0]);
    JHLK_100_Value.min = PUBLIC_IEEE754_32(recv->min[3], recv->min[2], recv->min[1], recv->min[0]);
    JHLK_100_Value.electric_quantity = PUBLIC_IEEE754_32(recv->electric_quantity[3], recv->electric_quantity[2], recv->electric_quantity[1], recv->electric_quantity[0]);
...
char *JHLK_100_RecvMessage(uint8_t *buff, uint16_t size)
{
    uint16_t crc, length;
    uint8_t tail;
    JHLK_100_MessageType *recv = (JHLK_100_MessageType *)buff;

    length = (recv->Length[0] << 8) | recv->Length[1];

    /* 防止数组越界 */
    if (length >= JHLK_100_DATA_SIZE)
        return NULL;
    ...
    printf("%x:%x\n", recv->Head, tail);
    /* 判断头尾 */
    if ((recv->Head != JHLK_100_MSG_HEAD) || (tail != JHLK_100_MSG_TAIL))
        return NULL;
    ...
    switch (recv->ControlType) {
        case JHLK_100_CONTROL_GET_PAST_DATA_CNT:
            /* 获取到数据条数 */
            return JHLK_100_AnalyDataCnt(recv->Data, length);

        case JHLK_100_CONTROL_GET_DETECTION_DATA:
            /* 采集到数据 */
            return JHLK_100_DataAnalysis(recv->Data, length);
    }
}
